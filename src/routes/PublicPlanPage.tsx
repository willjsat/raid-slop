...
export function PublicPlanPage() {
  ...
  return <div className="h-screen flex flex-col"><Header /><div className="p-2 border-b border-slate-800 text-sm flex items-center gap-2"><span className="font-semibold">{plan?.name ?? 'Plan'}</span><Link className="btn" to={plan ? `/planner/${plan.bossSlug}` : '/'}>Make a copy</Link></div><div className="flex-1 grid grid-cols-12 gap-2 p-2 overflow-hidden"><section className="col-span-9"><CanvasEditor readOnly /></section><aside className="panel col-span-3 p-2 overflow-auto"><StepPanel /></aside></div></div>;
}
