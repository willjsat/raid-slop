...
import { PlanSchema, type DrawableObject, type Plan } from '../types/planner';
...
export const usePlannerStore = create<PlannerState>()(
  persist(
    (set, get) => ({
      ...
      loadSeedPlanByBoss: (slug) =>
        set({
          currentPlan: structuredClone(seededByBoss[slug]),
          currentStepIndex: 0,
          selectedBoss: slug,
        }),
      ...
      loadPlanById: (id) =>
        set({
          currentPlan: structuredClone(get().plans[id]),
          currentStepIndex: 0,
        }),
      ...
      addStep: (copyPrevious) =>
        set((state) => {
          ...
          return {
            currentPlan: { ...state.currentPlan, steps, updatedAt: stamp() },
      ...
      moveStep: (index, direction) =>
        set((state) => {
          ...
          if (target < 0 || target >= state.currentPlan.steps.length) return state;
      ...
      addObject: (obj) =>
        set((state) => {
          ...
          steps[state.currentStepIndex] = {
            ...steps[state.currentStepIndex],
            objects: [...steps[state.currentStepIndex].objects, obj],
          };
          return {
            currentPlan: { ...state.currentPlan, steps, updatedAt: stamp() },
      ...
      updateObject: (id, patch) =>
        set((state) => {
          ...
          steps[state.currentStepIndex] = {
            ...current,
            objects: current.objects.map((o) => (o.id === id ? { ...o, ...patch } : o)),
          };
          return { currentPlan: { ...state.currentPlan, steps, updatedAt: stamp() } };
        }),
      ...
      deleteSelectedObject: () =>
        set((state) => {
          ...
          steps[state.currentStepIndex] = {
            ...current,
            objects: current.objects.filter((o) => o.id !== state.selectedObjectId),
          };
          return {
            currentPlan: { ...state.currentPlan, steps, updatedAt: stamp() },
            selectedObjectId: undefined,
      ...
      duplicateSelectedObject: () =>
        set((state) => {
          ...
          const current = state.currentPlan.steps[state.currentStepIndex];
          const selected = current.objects.find((o) => o.id === state.selectedObjectId);
          if (!selected) return state;

          const clone = { ...selected, id: uid(), x: selected.x + 20, y: selected.y + 20 };
          const steps = [...state.currentPlan.steps];
          steps[state.currentStepIndex] = { ...current, objects: [...current.objects, clone] };
      ...
      saveCurrentPlan: () =>
        set((state) => {
          ...
          return {
            plans: {
              ...state.plans,
              [state.currentPlan.id]: { ...state.currentPlan, updatedAt: stamp() },
            },
          };
        }),
      ...
      importPlan: (raw) => {
        try {
          ...
          set((state) => ({
            plans: { ...state.plans, [parsed.id]: parsed },
            currentPlan: parsed,
            currentStepIndex: 0,
          }));
          return { ok: true, message: 'Plan imported.' };
        } catch {
          return { ok: false, message: 'Invalid plan JSON.' };
        }
      },
...
const typeDefaults: Partial<Record<DrawableObject['type'], Partial<DrawableObject>>> = {
  'player-marker': { color: '#f59e0b', text: 'Player' },
  'group-marker': { color: '#60a5fa', text: 'G1' },
  'raid-marker': { color: '#c084fc', text: 'Raid' },
  'boss-marker': { color: '#fb7185', text: 'Boss' },
  'add-marker': { color: '#ef4444', text: 'Add' },
  'danger-zone': { color: '#ef4444', fill: 'rgba(239,68,68,.3)', text: 'Danger', width: 120, height: 120 },
  'safe-zone': { color: '#10b981', fill: 'rgba(16,185,129,.3)', text: 'Safe', width: 120, height: 120 },
  'text-label': { text: 'Callout', fill: 'rgba(15,23,42,.15)', width: 140 },
  'callout-badge': { text: '1', color: '#fde047', fill: 'rgba(234,179,8,.2)', width: 48, height: 48 },
};
